...
from .data import IMUData, FrameData, GPSTrack
...
def _extract_frame_records(tp: telemetry_parser.Parser) -> List[Dict[str, Any]]:
    ...
    return [{
        "timestamp_ms": frame["t"] * 1000,
        "exposure_time": frame["v"]
...
class MetadataCache:
    ...
    def __init__(self, cache_dir: Optional[Path | str] = DEFAULT_CACHE_DIR) -> None:
        self.cache_dir: Optional[Path] = (Path(cache_dir)
                                          if isinstance(cache_dir, str) else cache_dir)
...
def extract_metadata_cached(filepath: str | Path,
                            cache_dir=DEFAULT_CACHE_DIR,
                            use_telemetry_parser: bool = True) -> Dict[str, Any]:
...
def remap_imu_coordinates(angular_velocity, accelerometer, imu_orientation: str = "XYZ"):
    ...
    letter_to_sign = lambda a: 1 if a.isupper() else -1
...
def get_motion_data(metadata: str | Path | Dict[str, Any],
                    imu_orientation: str = None) -> Dict[str, Any]:
    ...
    raw_angular_velocity = (np.array([rec["gyro"] for rec in imu_records]) * np.pi / 180.0
                           )  # deg to rad
    ...
    angular_velocity, accelerometer = remap_imu_coordinates(raw_angular_velocity, raw_accelerometer,
                                                            imu_orientation)
    ...
    gps_data = (None if gps_records is None else GPSTrack(
